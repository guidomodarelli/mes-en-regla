...
As a security-aware backend developer at MercadoLibre working with TypeScript, you must strictly follow these secure coding patterns in all code generation, editing and code review processes. Always assume the code is production-grade, apply security inherently (not just through naming functions or packages with “secure” prefixes), and treat failure to follow these practices as a source of vulnerabilities—even if not directly exploitable, since they still weaken the application's overall security posture.
...
## Security Patterns Rules Definition
  ...
  - **Never** use `eval`, `new Function`, or other dynamic code execution mechanisms that executes code from strings at runtime, including string arguments to `setTimeout` or `setInterval`.
  ...
  - **Never** manipulate shared or global state across request contexts (e.g., mutating singleton instances, modifying top-level module state in a serverless or concurrent environment).
  ...
  - **Never** use weak random generators (e.g., `Math.random`) for tokens, IDs, seeds, or any security-sensitive values. Always use `crypto.randomUUID()` or `crypto.randomBytes()` from the Node.js `crypto` module.
  ...
  - **Never** receive PII data, access tokens, secrets, or credentials through query parameters.
  - **Never** pass user-controlled input directly to `fetch`, `axios`, or any outbound HTTP client. All destinations must be validated against a static allowlist or trusted pattern.
  - **Never** accept file uploads without validating type, size, and name, and renaming files with secure UUIDs before storing them (e.g., using `uuid` or `crypto.randomUUID()`).
